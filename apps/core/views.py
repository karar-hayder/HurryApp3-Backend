...
# from .utils.embeddings import extract_fingerprint_embedding
# from .utils.img_processing import process_fingerprint_image
...
class FingerPrintRegisterView(APIView):
    ...
    def post(self, request):
        ...
        is_known_value = request.data.get("is_known", "false")
        ...
        fingerprint = FingerPrint.objects.create(name=name, img=img_file, is_known=is_known)
...
#                     id__in=fingerprint_ids, img__isnull=False
#                 ).exclude(img="")

#             if not fingerprints.exists():
#                 return Response(
#                     {"error": "No fingerprints found with valid images to regenerate"}, status=404
#                 )
...
#                     logger.info(
#                         f"Deleted {deleted_count} old embeddings for fingerprint {fingerprint.id}"
#                     )

#                     # Check if image file exists
#                     if not fingerprint.img or not os.path.exists(fingerprint.img.path):
#                         results["errors"].append(
#                             {"fingerprint_id": fingerprint.id, "error": "Image file not found"}
#                         )
...
#                     logger.info(
#                         f"Generating embeddings for fingerprint {fingerprint.id} at path: {fingerprint.img.path}"
#                     )
#                     embeddings = process_fingerprint_image(
#                         fingerprint.img.path, method="minutiae", skip_complex_augmentations=True
#                     )
#                     logger.info(f"Embeddings type: {type(embeddings)}")
#                     logger.info(f"Embeddings is dict: {isinstance(embeddings, dict)}")
#                     if isinstance(embeddings, dict):
#                         logger.info(
#                             f"Generated {len(embeddings)} embedding variants for fingerprint {fingerprint.id}"
#                         )
#                     else:
#                         logger.error(
#                             f"Unexpected embeddings type: {type(embeddings)}, value: {embeddings}"
#                         )
#                         raise ValueError(f"Expected dict, got {type(embeddings)}")
...
#                             logger.warning(
#                                 f"Invalid embedding vector for {variant_name}: {type(vector)}"
#                             )
#                             continue
...
#                     logger.info(
#                         f"Successfully regenerated {new_embeddings_count} embeddings for fingerprint {fingerprint.id}"
#                     )

#                 except Exception as e:
#                     error_msg = f"Failed to regenerate embeddings for fingerprint {fingerprint.id}: {str(e)}"
#                     logger.error(error_msg)
#                     results["errors"].append({"fingerprint_id": fingerprint.id, "error": error_msg})
#                     results["total_errors"] += 1
...
#             return Response({"error": f"Failed to regenerate embeddings: {str(e)}"}, status=500)
...
#                 ).delete()[0]
#                 message = (
#                     f"Deleted {deleted_count} embeddings for {len(fingerprint_ids)} fingerprints"
#                 )
#             else:
#                 return Response(
#                     {"error": "Either provide fingerprint_ids or set delete_all=True"}, status=400
#                 )
...
#             return Response({"error": f"Failed to delete embeddings: {str(e)}"}, status=500)
