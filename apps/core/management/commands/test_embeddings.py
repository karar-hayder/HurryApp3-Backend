...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Test 1: Normal image with ORB
        self.stdout.write("\n1. Testing normal image with ORB...")
        test_image = np.random.randint(0, 255, (100, 100), dtype=np.uint8)
        try:
            embedding = get_embedding(test_image, use_orb=True)
            self.stdout.write(
                self.style.SUCCESS(
                    f"   ✓ Success: Generated embedding of length {len(embedding)}"
                )
            )
        ...
        try:
            embedding = get_embedding(uniform_image, use_orb=True)
            self.stdout.write(
                self.style.SUCCESS(
                    f"   ✓ Success: Generated embedding of length {len(embedding)}"
                )
        ...
        try:
            embedding = get_embedding(small_image, use_orb=True)
            self.stdout.write(
                self.style.SUCCESS(
                    f"   ✓ Success: Generated embedding of length {len(embedding)}"
                )
        ...
        try:
            ...
            embeddings = process_fingerprint_for_django(tmp_file_path, use_orb=True)
            self.stdout.write(
                self.style.SUCCESS(
                    f"   ✓ Success: Generated {len(embeddings)} embedding variants"
                )
            )
